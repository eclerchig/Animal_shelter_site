...
<?php
  if ($_SERVER['REQUEST_METHOD'] == 'POST' and isset($_POST["do_edit"]))
  {
    ...
    $query = "UPDATE users SET (surname,name,fathername,sex,age,work,num_telephone,address) VALUES ('$surname', '$name', '$fathername','$sex','$age','$work','$num','$address') WHERE ID = $_SESSION['id'];";

    // $query = "UPDATE `users` SET `surname` = '$surname', `name` = '$name',`fathername` = '$fathername', `sex` = '$sex',`age` = '$age', `work` = '$work',`num_telephone` = '$num',`address` = '$address'  WHERE `users`.`ID` = $_SESSION['id'];";
